...
public class RORepositoryTest {
    ...
    @Test
    public void findRepairOrdersByCafeArticle() throws Exception{
        //given
        Long caId = 1L;
        int idx=1;
        //when
        List<RepairOrder> result = roRepository.findRepairOrdersByCafeArticleId(caId);
        //then
        assertThat(result.size()).isEqualTo(8);
        for (RepairOrder ro : result) {
            assertThat(ro.getCause()).isEqualTo("cause" + idx);
            idx++;
        }
    }
}
